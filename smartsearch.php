...
//     "SELECT 
// bank.itemName
// FROM bank WHERE (((bank.count - (SELECT IFNULL(SUM(sold.count),'0') FROM sold WHERE sold.itemId=bank.itemId))>0) or ((case when (bank.revisedWeight != 'empty') THEN ABS(bank.revisedWeight - (SELECT IFNULL(sum(sold.weight),'0') FROM sold WHERE sold.itemId=bank.itemId) ) ELSE ABS(bank.weight - (SELECT IFNULL(sum(sold.weight),'0') FROM sold WHERE   sold.itemId=bank.itemId))  END)>0))");
